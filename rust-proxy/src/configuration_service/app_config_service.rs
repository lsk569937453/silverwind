...
async fn init_static_config() {
    ...
    let mut global_app_config = GLOBAL_APP_CONFIG.lock().await;
...
async fn init_app_service_config() -> Result<(), AppError> {
    let rw_app_config_read = GLOBAL_APP_CONFIG.lock().await;
    ...
    let mut rw_app_config_write = GLOBAL_APP_CONFIG.lock().await;
    ...
    for item in scrape_config.iter_mut() {
        ...
        tokio::spawn(async move {
            start_proxy(port, receiver, server_type, uuid).await;
        });
    }
